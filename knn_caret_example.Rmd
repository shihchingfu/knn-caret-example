...
According to the Youden criterion on the training set, the best threshold is 0.4.


# kNN classification

Apply the final model, with k = 31 and cutoff = 0.4, to the testing dataset to get an estimate of the true performance of this classifier.

```{r}
knn_predictions <- predict(knn_cv, newdata = testing_set, type = "prob") %>%
  select(probability = genuine) %>%
  mutate(class = ifelse(probability > 0.4, "genuine", "counterfeit")) %>%
