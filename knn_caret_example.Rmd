...
Inspecting the probabilities reveals that any cutoff somewhere in [0.5,0.8] will give about the same classification results.

```{r}
testing_set <- testing_set %>%
  mutate(Predicted_prob = predict(knn_cv, newdata = testing_set, type = "prob")$genuine)
...
```{r}
pROC_obj <- roc(testing_set$Status, testing_set$Predicted_prob,
                quiet = TRUE,
                plot = TRUE, 
                percent = TRUE,
                auc.polygon = TRUE, 
